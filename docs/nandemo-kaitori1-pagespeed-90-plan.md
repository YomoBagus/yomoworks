...
**ここが本件の最大の分岐点です。** 手元の端末で以下を実行してください。
...
- **② が空 → ルートは静的HTML。**
  → **WP Rocket はトップページに対して一切効きません。** §4（ケースA）へ。
- **② に `wp-content` 等が出る → ルートも WordPress。**
  → WP Rocket が使えます。§5（ケースB）へ。
...
> 静的HTMLページには適用されません。
>
> つまり推定が正しければ、**「WP Rocket を入れてトップページを90点にする」は原理的に不可能**で、
> 効くのは `/wp/` 配下のブログ記事だけです。まずここを確定させないと、
> 設定を詰めても数値が1点も動かないという事態になります。
...
## 5. ケースB：ルートも WordPress だった場合（WP Rocket 本命設定）

②の判定で `wp-content` が出た場合のみ、以下が有効です。上から順に、**1項目ずつ有効化 →
表示確認 → 計測** を繰り返してください。まとめて有効化すると事故の切り分けができません。
...
| Step | 内容 | 理由 |
|---|---|---|
| **0** | §1 の構成判定 + ベースライン計測 | **これをやらないと以降が全部空振りする可能性** |
...
1. **最重要**: ルートが静的HTMLなら、**WP Rocket はトップページのスコアを1点も動かしません。**
   §1 の判定を最初に必ず実施してください。
