...
>
> - **WP Rocket はトップページに適用されません**（静的HTMLのため構造上不可）。対応は **§4（ケースA）** で進めます。
> - **WP Rocket は本件では使用しません。** クライアント確認により、利用しない方針で確定しました。
> - **§5（ケースB）は不適用**です。将来 WordPress 化した場合の参考知識として残しています。
> - **§4 の施策は有料プラグインに一切依存しません。** サーバー標準機能とサイト改修のみで完結します。
...
> **本件では適用されません。**トップページが静的HTMLであることに加え、
> WP Rocket 自体を使用しない方針で確定したためです。
> 将来 WordPress 化する場合、または他案件での参考知識として残します。
>
> `/wp/` 配下のブログを軽くする場合は、WP Rocket ではなく
> **エックスサーバーの「サーバーキャッシュ設定」＋無料の画像圧縮プラグイン**で対応します。
...
1. **確定**: ルートは静的HTMLで、**WP Rocket は使用しません。**
   §4（ケースA）で対応します。**有料プラグインなしで 90 点を目指せる構成**です。
